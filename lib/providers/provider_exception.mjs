...
export class ProviderException extends Error {
    constructor(innerException) {
        ...
        if(innerException instanceof ProviderException) {
            // Preserve the original name of this error class (Not ProviderException)
            // This occurs when we explicitly throw a ProviderException and then have to rethrow.
            this.nameOfInnerException = innerException.nameOfInnerException
            ...
            this.interpretedErrorType = innerException.interpretedErrorType;
            this.interpretedCode = innerException.interpretedCode;
            this.interpretedMessage = innerException.interpretedMessage;
            this.details = innerException.details;
            return
        }
...
export function isStructuredProviderExceptionJson(errorJson) {
    return (
        errorJson !== null
        && typeof errorJson === 'object'
        && !Array.isArray(errorJson)
        && (
            typeof errorJson.provider === 'string'
            || typeof errorJson.interpretedErrorType === 'string'
            || typeof errorJson.nameOfInnerException === 'string'
        )
    );
}

export function providerExceptionFromJson(errorJson, fallbackStatus = undefined) {
    if (!isStructuredProviderExceptionJson(errorJson)) {
        return null;
    }

    const innerException = new Error(
        errorJson.message
        || errorJson.interpretedMessage
        || `HTTP ${fallbackStatus ?? 500}`
    );
    const ex = new ProviderException(innerException);
    ...
    if (ex.status === undefined && fallbackStatus !== undefined) {
        ex.status = fallbackStatus;
    }

...
class OpenAIChatModel extends ChatModel {
      ...
      try {
        ...
        if (stream) {
          ...
          console.log({"event":"about to stream complete",
            timeout: invocationOptions.ms_timeout || self.ms_timeout
          })
          response = await withRetryLogging(
            ...
            { maxAttempts: invocationOptions.max_attempts || self.max_attempts });
          ...
          try {
            console.log({"event":"about to non-stream complete",
              timeout: invocationOptions.ms_timeout || self.ms_timeout,
              optionsRequest: invocationOptions,
              timeoutRequest: invocationOptions.ms_timeout,
              timeoutConfig: self.ms_timeout
            })
            response = await withRetryLogging(
                () => this.client.chat.completions.create(
                  params,
  ...
  _isContextOverflow(error) {
    const message = (error.message || '').toLowerCase();
    return (error.code == 400 && message.includes("maximum context length"))
  }

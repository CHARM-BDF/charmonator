...
const reContextOverflow = new RegExp(
  "maximum context length"
  +"|too many tokens"
, 'i');

function isAbortLikeError(error, abortSignal = null) {
  return Boolean(
    abortSignal?.aborted
...
class OpenAIChatModel extends ChatModel {
  ...
  async extendTranscript(prefix, callOnOutput = null, suffix = null, streamOrOptions = false) {
    ...
    if (this.model === 'o1-preview') {
    ...
    } else if (this.model === 'gpt-5' || this.model === 'gpt-5-mini') {
      ...
      // Let's check for a verbosity:
      if (this.modelConfig.verbosity) {
        params.verbosity = this.modelConfig.verbosity;
    }
        ...
        if (stream) {
          ...
          try {
            ...
            if (!response?.choices || response.choices.length === 0) {
              logToFile("API Response received but no choices were returned (defective response):", response);
              const content = []   // !!: See num_defective_reply_max_attempts just up on the call stack.
              return suffix.plus(new Message('assistant', content))
            } else {
            ...
            if (isAbortLikeError(error, invocationOptions.abort_signal)) {
              throw error;
            }
  ...
  _isContextOverflow(error) {
    ...
    const t = reContextOverflow.test(message);
    return (status === 400 && t)
  }

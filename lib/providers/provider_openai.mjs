...
import { Message, TranscriptFragment, ToolCall, ToolResponse, Attachment, 
         ImageAttachment, DocumentAttachment } from '../transcript.mjs';
...
class OpenAIChatModel extends ChatModel {
  ...
  async extendTranscript(prefix, callOnOutput = null, suffix = null, stream = false) {
    ...
    // Some new or specialized models (like `o1`) may not support certain params:
    if (this.model === 'o1' || this.model === 'o1-preview' || this.model === 'o3-mini') {
      delete params.temperature;
      delete params.stream;
      // Reasoning param, if your config needs it
      params.reasoning_effort = 'high';

      // Check if model config had a reasoning effort:
      ...
      try {
        ...
        if (stream) {
          ...
          if (assistantMessage?.function_call) {
            ...
            return await this.extendTranscript(prefix, callOnOutput, newSuffix, stream);
          } else {

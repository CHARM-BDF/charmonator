...
export function tokenCount(text, encodingName = "cl100k_base") {
  // TODO: what is the use case for a custom tokenCounter?
  if (!tiktokenLib) {
    throw new Error(
      `No tokenCounter was provided, and "tiktoken" is not available. ` +
      `If running in the browser, you must supply a tokenCounter.`
  ...
  const enc = tiktokenLib.get_encoding(encodingName);
  return enc.encode(text).length;
}

...
import charmonatorTranscriptExtensionRouter from '../routes/transcript-extension.mjs';
...
import charmonizerTranscriptExtensionRouter from '../routes/charmonizer/transcript-extension.mjs';
...
export async function createAndStart() {
  try {
    ...
    app.use(CHARMONATOR_API_PREFIX + "/transcript", charmonatorTranscriptExtensionRouter);
    ...
    app.use(CHARMONIZER_API_PREFIX + '/transcript/extension', charmonizerTranscriptExtensionRouter);
    ...
    async function cleanup({ force = false } = {}) {
      forceCloseRequested = forceCloseRequested || force;
    ...
    // Return both the server and a cleanup function
    return {
      server: server,

      // Add a cleanup function that properly shuts down MCP servers
      cleanup
    };
  } catch (error) {

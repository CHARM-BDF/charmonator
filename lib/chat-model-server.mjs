...
export class ChatModel {
  constructor(provider, client, modelConfig) {
    ...
    // ephemeral invocation-time options can be stored here if desired
    this.options = {};
    ...
    // Supported formats:
    // - Legacy: tools: ["t1", "t2"]
    // - New: tools: { server: [...], client: [...], mcp: [...] }
    const toolCfg = modelConfig.tools;
    const toolNames = [];
    if (Array.isArray(toolCfg)) {
      toolNames.push(...toolCfg);
    } else if (toolCfg && typeof toolCfg === 'object') {
      if (Array.isArray(toolCfg.server)) toolNames.push(...toolCfg.server);
      if (Array.isArray(toolCfg.client)) toolNames.push(...toolCfg.client);
      if (Array.isArray(toolCfg.mcp)) toolNames.push(...toolCfg.mcp);
    }
    for (const toolName of toolNames) {
      this.enableTool(toolName);
    }

    // Legacy compatibility: allow legacy configs to specify MCP tools separately.
    for (const toolName of modelConfig._mcp_tools || []) {
      this.enableTool(toolName);
    }
  }
  ...
  /**
   * Add a tool at runtime (e.g. ephemeral or from user-provided config).
   */
  addTool(tool) {
   ...
   * 
   * Historically, the 4th parameter was a boolean `stream`. Now, you can
   * pass either `true/false` or an `options` object that may contain:
   *   { stream?: boolean, response_format?: object, etc. }
   */

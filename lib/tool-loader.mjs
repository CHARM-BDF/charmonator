...
export function validateModelToolRefs(config) {
  ...
  for (const [modelName, modelCfg] of Object.entries(config.models)) {
    ...
    // New format: tools: { server: [], client: [], mcp: [] }
    // Compatibility: allow tools: ["t1", ...] as shorthand.
    const flatRefs = Array.isArray(toolRefs) ? toolRefs : [];
    const serverRefs = Array.isArray(toolRefs.server) ? toolRefs.server : flatRefs;
    const clientRefs = Array.isArray(toolRefs.client) ? toolRefs.client : [];
    const mcpRefs = Array.isArray(toolRefs.mcp) ? toolRefs.mcp : [];

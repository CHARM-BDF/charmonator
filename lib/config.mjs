...
function populateModelConfigDefaults(configObj, configGlobal) {
  ...
  if (configObj.max_attempts == undefined) {
    configObj.max_attempts = configGlobal.max_attempts || 2;
  }
  if (configObj.ms_timeout == undefined) {
    configObj.ms_timeout = configGlobal.ms_timeout || 600000;
  }
  return configObj;
}
...
function validateModelConfigDefaults(configObj) {
  ...
  configObj.ms_timeout = Number(configObj.ms_timeout)
  if(Number.isNaN(configObj.ms_timeout) || configObj.ms_timeout <= 0) {
    throw new Error("configuration: ms_timeout must be a positive number")
  }
}

...
function populateConfigDefaults(configObj) {
  ...
  if (configObj.num_schema_repair_max_attempts === undefined) {
    configObj.num_schema_repair_max_attempts = 5;
  }
  if (configObj.num_defective_reply_max_attempts === undefined) {
    configObj.num_defective_reply_max_attempts = 5;
  }
  ...
  if (configObj.num_client_request_max_attempts == undefined) {
    configObj.num_client_request_max_attempts = 2;
  }
  if (configObj.ms_client_request_timeout == undefined) {
    configObj.ms_client_request_timeout = 600000;
  }
  return configObj;
}

function populateModelConfigDefaults(configModel, configGlobal) {
  if (configModel.num_schema_repair_max_attempts == undefined) {
    configModel.num_schema_repair_max_attempts = configGlobal.num_schema_repair_max_attempts;
  }
  if (configModel.num_defective_reply_max_attempts == undefined) {
    configModel.num_defective_reply_max_attempts = configGlobal.num_defective_reply_max_attempts;
  }
  if (configModel.num_client_request_max_attempts == undefined) {
    configModel.num_client_request_max_attempts = configGlobal.num_client_request_max_attempts;
  }
  if (configModel.ms_client_request_timeout == undefined) {
    configModel.ms_client_request_timeout = configGlobal.ms_client_request_timeout;
  }
  return configModel;
}

function validateModelConfigDefaults(configObj) {
  configObj.num_client_request_max_attempts = Number(configObj.num_client_request_max_attempts)
  if(Number.isNaN(configObj.num_client_request_max_attempts) || configObj.num_client_request_max_attempts <= 0) {
    throw new Error(`configuration: num_client_request_max_attempts must be a postive number: ${configObj.num_client_request_max_attempts}`)
  }
...
export function getModelConfig(modelName) {
  ...
  // Resolve model aliases (strings that point to other model keys)
  let resolvedModelName = selectedModelName;
  const visited = new Set();
  while (typeof cfg.models[resolvedModelName] === 'string') {
    if (visited.has(resolvedModelName)) {
      throw new Error(`Circular model alias detected: ${[...visited, resolvedModelName].join(' -> ')}`);
    }
    visited.add(resolvedModelName);
    resolvedModelName = cfg.models[resolvedModelName];
  }

  let modelConfig = cfg.models[resolvedModelName];
  if (!modelConfig) {
  ...
  const api_key = (
    ((cfg.secret?.models || {})[resolvedModelName] || {})["api_key"] ||
    ((cfg.models || {})[resolvedModelName] || {})["api_key"]
  );

  return {
    ...modelConfig,
    api_key: interpolateEnvVars(api_key),

...
function parseAndPopulateSecretConfig(filePath, config) {
  ...
  for (const k of Object.keys(models)) {
    if(isModel(models[k])) {
      let envkey = "CHARMONATOR_"+k+"_API_KEY"
      if(envkey in process.env) {
        models[k]["api_key"] = process.env[envkey]
      }
    }
  }
}
...
function parseAndPopulateConfig(filePath) {
  ...
  parseAndPopulateSecretConfig(filePath, config)
  for (const k of Object.keys(models)) {
    if(isModel(models[k])) {

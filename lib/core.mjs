...
import { GoogleProvider } from './providers/provider_google.mjs';
import { TestPolicyProvider } from './providers/provider_test_policy.mjs';
import { providerExceptionFromJson } from './providers/provider_exception.mjs';
...
export function getProviderName(modelConfig) {
  const c = fetchProviderConstructor(modelConfig)
  return c.name
}
...
export async function imageToMarkdown({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      /*
      Design choice for loopback requests:
        The code must either:
        1) Rehydrate/deserialize exceptions from loopback requests to contain interpretedMessage, status, class==ProviderException and other important metadata.
        2) Have liberal exception handlers that aren't aware of these things.
      We choose 1).  Therefore we call providerExceptionFromJson.
      For a typical example of where this matters, see the following call path:
        processPdfDocument
        => fallbackVisionModel
        => imageToMarkdown (You are here.  We loopback POST /conversion/image)
      */

      const structuredError = providerExceptionFromJson(errorJson, response.status);
      if (structuredError) {
...
/**
 * Calls the `transcript/extension` endpoint and wraps it as `extendTranscript`.
 *
...
export async function extendTranscript(model, transcriptPrefix) {
  ...
  const port = getServerPort();
  const absoluteUrl = `http://localhost:${port}`;
  const fullUrl = absoluteUrl + getFullCharmonatorApiPrefix() + '/transcript/extension';
  ...
  try {
    ...
    console.error('Error calling transcript/extension:', error);
    throw error;
  }
}
